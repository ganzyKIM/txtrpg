...
- ✅ **B. 프록시·과금**: Edge Function `generate-text/image`, 키 서버 이전, 크레딧 차감/장부
- ✅ **C. 저장 이전**: 세이브를 DB로, 자동저장, 이미지 Storage 이전
- ✅ **D. 백오피스**: 유저관리·충전·내역·config 편집
- ⬜ **E. 출시 점검**: CSAM 최소 가드, 약관/환불 안내, 무료티어 한계 점검 — **미착수, 출시 전 필수**

> 내부 프롬프트 영어화(5장)는 보류. 현재 한국어 프롬프트로 출력 품질에 문제가 없어 우선순위를 내렸다.
...
```
변경 이력
- v3.1 (2026-08): A~D 구현 완료 표기. 프롬프트 영어화 보류.
