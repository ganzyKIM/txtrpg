...
6. [위기 이벤트] 매 응답마다 내면적으로 주사위를 굴려 약 5% 확률로 '위기 이벤트'를 발동하세요. 위기는 지금 이야기의 흐름과 세계관·인물 관계에서 자연스럽게 비롯되어야 하며, 주인공을 갑작스럽게 당황시키고 긴장감을 높이는 사건이어야 합니다. 완전히 뜬금없이 발생하지 않도록 복선을 한두 줄 깔고 터뜨리세요. 위기 이벤트가 발동되지 않은 95%의 응답에서는 이 지침을 일절 언급하거나 암시하지 마세요.`);
